...
const SubnetsPage = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const subnetsRes = await apiClient.get('/compute/v1/projects/demo-project/aggregated/subnetworks');
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post(
        `/compute/v1/projects/demo-project/regions/${formData.region}/subnetworks`,
        {
          name: formData.name,
  ...
  const handleDelete = async (region: string, subnetName: string) => {
    ...
    try {
      await apiClient.delete(
        `/compute/v1/projects/demo-project/regions/${region}/subnetworks/${subnetName}`
      );
      await loadData();
    } catch (error: any) {
