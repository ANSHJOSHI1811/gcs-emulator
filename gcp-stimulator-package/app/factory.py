...
def wait_for_database(app: Flask) -> None:
    ...
    delay_seconds = float(os.getenv("DB_CONNECT_RETRY_DELAY_SECONDS", "1"))
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            last_error = exc
            app.logger.warning(
                f"Database not ready (attempt {attempt}/{retries}); retrying in {delay_seconds}s"
            )
...
def register_blueprints(app: Flask) -> None:
    ...
    app.register_blueprint(iam_bp)  # IAM service accounts and policies (routes include /v1)
    ...
    app.register_blueprint(project_bp)  # Cloud Resource Manager (Projects)
...
def register_sdk_middleware(app: Flask) -> None:
    ...
    def handle_sdk_auth():
        ...
        if app.config.get('MOCK_AUTH_ENABLED', True):
            ...
            # Accept any authorization (stimulator mode)
            # In real GCS, this would validate OAuth2 tokens
            pass
    
    @app.route('/storage/v1')
    def sdk_info():
        """SDK discovery endpoint - helps verify stimulator is running"""
        return jsonify({
