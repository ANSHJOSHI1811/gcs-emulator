...
from app.models.database import get_db, GKECluster, GKENodePool, GKEAddon, SessionLocal
...
def _provision_cluster(cluster_id: int, op_id: Optional[str] = None):
    ...
    from app.core.docker_manager import create_k3s_cluster, get_k3s_kubeconfig
...
def _delete_cluster_bg(cluster_id: int, container_id: Optional[str], cluster_name: str, op_id: Optional[str] = None):
    from app.core.docker_manager import delete_k3s_cluster
...
def _stop_cluster_bg(cluster_id: int, container_id: str, op_id: Optional[str] = None):
    from app.core.docker_manager import stop_k3s_cluster
...
def _start_cluster_bg(cluster_id: int, container_id: str, op_id: Optional[str] = None):
    from app.core.docker_manager import start_k3s_cluster
    import time
...
def _delete_nodepool_bg(nodepool_id: int, container_ids: list, op_id: Optional[str] = None):
    from app.core.docker_manager import delete_k3s_agent
...
def _provision_nodepool_bg(np_id: int, cluster_id: int, node_count: int, pool_name: str, op_id: Optional[str] = None):
    ...
    from app.core.docker_manager import create_k3s_agents
    import time
...
def _resize_nodepool_bg(np_id: int, cluster_id: int, desired: int, op_id: Optional[str] = None):
    ...
    from app.core.docker_manager import resize_k3s_agents
...
def exec_kubectl(
    ...
    from app.core.docker_manager import run_kubectl_command
...
# ─── Sprint 4: Workloads, Pod Logs, Namespaces, Services, YAML Apply ──────────
...
def _kubectl_for_cluster(cluster: GKECluster, *args) -> subprocess.CompletedProcess:
    """Run kubectl against the cluster's container using its kubeconfig."""
    container_id = cluster.container_id
    if not container_id:
        raise HTTPException(503, f"Cluster {cluster.name} has no running container")
    cmd = ["docker", "exec", container_id, "kubectl"] + list(args)
    result = subprocess.run(cmd, capture_output=True, text=True, timeout=15)
    return result


@router.get("/projects/{project}/locations/{location}/clusters/{cluster_name}/workloads")
def list_workloads(project: str, location: str, cluster_name: str,
                   namespace: str = "all", db: Session = Depends(get_db)):
    """
    Sprint 4: List pods, deployments, statefulsets across all namespaces.
    Returns structured { pods: [...], deployments: [...], statefulSets: [...] }
    """
    cluster = db.query(GKECluster).filter_by(
        project_id=project, location=location, name=cluster_name
    ).first()
    if not cluster:
        raise HTTPException(404, "Cluster not found")
    if cluster.status != "RUNNING":
        raise HTTPException(503, "Cluster is not running")

    ns_flag = ["--all-namespaces"] if namespace == "all" else ["-n", namespace]

    # Pods
    pods_result = _kubectl_for_cluster(cluster, "get", "pods", *ns_flag, "-o", "json")
    deployments_result = _kubectl_for_cluster(cluster, "get", "deployments", *ns_flag, "-o", "json")
    statefulsets_result = _kubectl_for_cluster(cluster, "get", "statefulsets", *ns_flag, "-o", "json")

    def _parse(result):
        try:
            return _json.loads(result.stdout).get("items", [])
        except Exception:
            return []
    ...
    def _deployment_summary(d: dict) -> dict:
        meta = d.get("metadata", {})
        spec = d.get("spec", {})
        status = d.get("status", {})
        return {
    ...
    return {
        "pods":         [_pod_summary(p)    for p in _parse(pods_result)],
        "deployments":  [_deployment_summary(d) for d in _parse(deployments_result)],
        "statefulSets": [_ss_summary(s)     for s in _parse(statefulsets_result)],
    }


@router.get(
    "/projects/{project}/locations/{location}/clusters/{cluster_name}"
    "/namespaces/{namespace}/pods/{pod_name}/logs"
)
def get_pod_logs(project: str, location: str, cluster_name: str,
                 namespace: str, pod_name: str,
                 container: str = None, tail: int = 200,
                 db: Session = Depends(get_db)):
    """Sprint 4: Stream tail of pod logs via kubectl logs."""
    cluster = db.query(GKECluster).filter_by(
        project_id=project, location=location, name=cluster_name
    ).first()
    if not cluster:
        raise HTTPException(404, "Cluster not found")
    if cluster.status != "RUNNING":
        raise HTTPException(503, "Cluster not running")

    args = ["logs", pod_name, "-n", namespace, f"--tail={tail}"]
    if container:
        args += ["-c", container]

    result = _kubectl_for_cluster(cluster, *args)
    return {"logs": result.stdout or result.stderr, "podName": pod_name, "namespace": namespace}


@router.get("/projects/{project}/locations/{location}/clusters/{cluster_name}/namespaces")
def list_namespaces(project: str, location: str, cluster_name: str, db: Session = Depends(get_db)):
    """Sprint 4: List Kubernetes namespaces."""
    cluster = db.query(GKECluster).filter_by(
        project_id=project, location=location, name=cluster_name
    ).first()
    if not cluster:
        raise HTTPException(404, "Cluster not found")
    result = _kubectl_for_cluster(cluster, "get", "namespaces", "-o", "json")
    try:
    ...
    return {
        "namespaces": [{
            ...
            "creationTimestamp": ns.get("metadata", {}).get("creationTimestamp"),
            "labels": ns.get("metadata", {}).get("labels", {}),
        } for ns in items]
    }


@router.post("/projects/{project}/locations/{location}/clusters/{cluster_name}/namespaces")
def create_namespace(project: str, location: str, cluster_name: str,
                     body: dict, db: Session = Depends(get_db)):
    """Sprint 4: Create a Kubernetes namespace."""
    cluster = db.query(GKECluster).filter_by(
        project_id=project, location=location, name=cluster_name
    ).first()
    if not cluster:
        raise HTTPException(404, "Cluster not found")
    ns_name = body.get("name")
    if not ns_name:
        raise HTTPException(400, "name is required")
    result = _kubectl_for_cluster(cluster, "create", "namespace", ns_name)
    if result.returncode != 0 and "already exists" not in result.stderr:
        raise HTTPException(500, result.stderr)
    return {"name": ns_name, "status": "Active"}


@router.delete("/projects/{project}/locations/{location}/clusters/{cluster_name}/namespaces/{namespace}")
def delete_namespace(project: str, location: str, cluster_name: str,
                     namespace: str, db: Session = Depends(get_db)):
    """Sprint 4: Delete a Kubernetes namespace."""
    cluster = db.query(GKECluster).filter_by(
        project_id=project, location=location, name=cluster_name
    ).first()
    if not cluster:
    ...
    if namespace in ("default", "kube-system", "kube-public"):
        raise HTTPException(400, f"Cannot delete system namespace: {namespace}")
    result = _kubectl_for_cluster(cluster, "delete", "namespace", namespace)
    if result.returncode != 0:
        raise HTTPException(500, result.stderr)
    return {"deleted": namespace}


@router.get("/projects/{project}/locations/{location}/clusters/{cluster_name}/services")
def list_k8s_services(project: str, location: str, cluster_name: str,
                      namespace: str = "all", db: Session = Depends(get_db)):
    """Sprint 4: List Kubernetes Services."""
    cluster = db.query(GKECluster).filter_by(
        project_id=project, location=location, name=cluster_name
    ).first()
    if not cluster:
        raise HTTPException(404, "Cluster not found")
    ns_flag = ["--all-namespaces"] if namespace == "all" else ["-n", namespace]
    result = _kubectl_for_cluster(cluster, "get", "services", *ns_flag, "-o", "json")
    try:
        items = _json.loads(result.stdout).get("items", [])
    except Exception:
        items = []
    ...
    def _svc(s: dict) -> dict:
        ...
        lb_ip = ""
        ingresses = status.get("loadBalancer", {}).get("ingress", [])
        if ingresses:
            lb_ip = ingresses[0].get("ip") or ingresses[0].get("hostname", "")
        return {
            "name": meta.get("name"),
            "namespace": meta.get("namespace"),
            "type": spec.get("type", "ClusterIP"),
            "clusterIP": spec.get("clusterIP"),
            "externalIP": lb_ip or spec.get("externalIPs", [None])[0],
            "ports": ports,
            "selector": spec.get("selector", {}),
            "creationTimestamp": meta.get("creationTimestamp"),
        }

    return {"services": [_svc(s) for s in items]}


@router.post("/projects/{project}/locations/{location}/clusters/{cluster_name}/apply")
def apply_yaml(project: str, location: str, cluster_name: str,
               body: dict, db: Session = Depends(get_db)):
    """Sprint 4: Apply arbitrary Kubernetes YAML via kubectl apply -f -."""
    cluster = db.query(GKECluster).filter_by(
        project_id=project, location=location, name=cluster_name
    ).first()
    if not cluster:
    ...
    yaml_content = body.get("yaml") or body.get("manifest", "")
    if not yaml_content:
        raise HTTPException(400, "yaml or manifest field required")

    container_id = cluster.container_id
    # Write the YAML and kubectl apply via stdin
    cmd = [
        "docker", "exec", "-i", container_id,
        "kubectl", "apply", "-f", "-"
    ]
    result = subprocess.run(cmd, input=yaml_content, capture_output=True, text=True, timeout=30)
    if result.returncode != 0:
...
@router.post("/projects/{project}/locations/{location}/clusters/{cluster_name}:upgrade")
def upgrade_cluster(project: str, location: str, cluster_name: str,
                    body: dict, db: Session = Depends(get_db)):
    """Sprint 4: Upgrade cluster master version."""
    cluster = db.query(GKECluster).filter_by(
        project_id=project, location=location, name=cluster_name
    ).first()
    if not cluster:
        raise HTTPException(404, "Cluster not found")
    new_version = body.get("masterVersion") or body.get("desiredMasterVersion")
    if not new_version:
        raise HTTPException(400, "masterVersion required")
    if new_version not in _SUPPORTED_VERSIONS:
        raise HTTPException(400, f"Unsupported version {new_version}. Supported: {_SUPPORTED_VERSIONS}")
    cluster.master_version = new_version
    db.commit()
    op_id = str(random.randint(10 ** 12, 10 ** 13 - 1))
    return {
        "name": op_id,
...
@router.get("/projects/{project}/locations/{location}/clusters/{cluster_name}/events")
def list_cluster_events(project: str, location: str, cluster_name: str,
                        db: Session = Depends(get_db)):
    """Sprint 4: List Kubernetes events across all namespaces."""
    cluster = db.query(GKECluster).filter_by(
        project_id=project, location=location, name=cluster_name
    ).first()
    if not cluster:
        raise HTTPException(404, "Cluster not found")
    result = _kubectl_for_cluster(cluster, "get", "events", "--all-namespaces",
    ...
    def _evt(e: dict) -> dict:
        meta = e.get("metadata", {})
        rr = e.get("involvedObject", {})
        return {
            "namespace": meta.get("namespace"),
            "reason": e.get("reason"),
            "message": e.get("message"),
            "type": e.get("type"),            # Normal | Warning
            "count": e.get("count", 1),
            "firstTime": e.get("firstTimestamp"),
            "lastTime": e.get("lastTimestamp"),
            "object": f"{rr.get('kind','')}/{rr.get('name','')}",
        }

    return {"events": [_evt(e) for e in items[-100:]]}  # cap at last 100
